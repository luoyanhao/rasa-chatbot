...
class FramePolicy(Policy):
    ...
    def __init__(
        ...
        """
        super(FramePolicy, self).__init__(priority=priority)
    ...
    def persist(self, path: Text) -> None:
        ...
        config_file = os.path.join(path, "frame_policy.json")
        meta = {"priority": self.priority}
        rasa.utils.io.create_directory_for_file(config_file)
        utils.dump_obj_as_json_to_file(config_file, meta)

    @classmethod
    def load(cls, path: Text) -> "FramePolicy":
        """Returns the class with the configured priority."""

        meta = {}
        if os.path.exists(path):
            meta_path = os.path.join(path, "frame_policy.json")
